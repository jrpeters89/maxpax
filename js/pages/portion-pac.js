...
function packagingCheck(user_token, company_id) {
	...
	$.get("/src/packaging_inventory.php?act=list&user_token="+user_token+"&company_id="+company_id,function(result) {
		...
		if(inventory.count > 0) {
			...
			jQuery.each( inventory.data, function( i, inv ) {
				$("#packaging_inventory_list").append('<div id="inventory_list" class="table-responsive"><table id="inventory_list_table_' + inv.ItemId +'" class="table sortable"><thead><tr><th class="width_100" data-defaultsort="asc"></th><th class="width_180"></th><th>Batch #</th><th class="width_100">Exp. Date</th><th class="text_right">Quantity</th><th>UOM</th><th>Location</th></tr></thead><tbody><tr><td>' + inv.ItemGroupId + '</td><td>' + inv.ItemId + '</td><td>' + inv.ItemName + '</td><td></td><td></td><td></td><td></td></tr></tbody></table></div>');
				...
				jQuery.each(inv, function(x, item) {
					if (typeof item.BatchNumber != undefined) {
						$("#inventory_list_table_" + inv.ItemId + " tbody").append('<tr><td></td><td></td><td>' + item.BatchNumber + '</td><td></td><td></td><td></td><td></td></tr>');
					}
				});
			})
			$.bootstrapSortable(false);
