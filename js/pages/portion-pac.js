	...
	$.get("/src/inventory.php?act=list&user_token="+user_token,function(result) {
		...
		if(inventory.count > 0) {
			...
			jQuery.each( inventory.data, function( i, inv ) {
				$("#inventory_list tbody").append('<tr><td class="width_180">'+inv.ItemId+'</td><td>'+inv.ItemName+'</td><td class="text_right">'+inv.AvailPhysical+'</td><td>'+inv.BOMUnitId+'</td><td>'+inv.Case+'</td><td>'+inv.SellUOM+'</td><td>'+inv.Pallet+'</td><td>'+inv.Location+'</td></tr>');
			});
			if(typeof inventory.total != "undefined") {
				$("#inventory_list tfoot").append('<tr><td colspan="2">TOTAL</td><td>'+inventory.total.quantity+'</td><td></td><td>'+inventory.total.quantity.case+'</td><td></td><td>'+inventory.total.quantity.pallet+'</td></tr>');
			}
