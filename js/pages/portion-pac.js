	...
	$.get("/src/inventory.php?act=list&user_token="+user_token,function(result) {
		...
		if(inventory.count > 0) {
			...
			jQuery.each( inventory.data, function( i, inv ) {
				$("#inventory_list tbody").append('<tr><td class="width_180">'+inv.ItemId+'</td><td>'+inv.ItemName+'</td><td class="text_right">'+inv.AvailPhysical+'</td><td>'+inv.BOMUnitId+'</td><td></td><td></td><td></td><td>'+inv.Location+'</td></tr>');
			});
