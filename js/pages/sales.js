		...
		if (aging.data.total > 0) {
			var plot = $.plot($("#chart_box"), [ {
					...
					bars: {
						...
						numbers: {
							show: true,
							yAlign: function (y) { return y + 2.5; }
						}
					},
					yaxis: {
							show: true,
			...
			$("#chart_box").append('<div style="padding-top: 255px; text-align: center; font-size: 12px;">Grand Total: '+numeral(Math.round(aging.data.chart[0][1] + aging.data.chart[1][1] + aging.data.chart[2][1] + aging.data.chart[3][1] + aging.data.chart[4][1] + aging.data.chart[5][1] )).format('0,0')+'</div>');
