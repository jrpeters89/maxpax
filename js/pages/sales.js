...
function agingChart(user_token,company) {
	...
	$.get("/src/cust_aging.php?user_token="+user_token+"&company="+company,function(result) {
		...
		if (aging.data.total > 0) {
			...
			$("#chart_box").append('<div style="padding-top: 305px; text-align: center; font-size: 12px;">Grand Total: '+numeral(Math.round(aging.data.chart[0][1] + aging.data.chart[1][1] + aging.data.chart[2][1] + aging.data.chart[3][1] + aging.data.chart[4][1] + aging.data.chart[5][1] )).format('0,0')+'</div>');
