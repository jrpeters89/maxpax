...
function hershey(user_token, company_id) {
    ...
    $.get("/src/hershey_documents.php?user_token=" + user_token + "&start_date=" + startDateTxt.value + "&end_date=" + endDateTxt.value + "&company_id=" + company_id, function (result) {
       var hershey = jQuery.parseJSON(result);
        ...
        if (hershey.count > 0) {
            $("#hershey_list").html('');
            $("#hershey_list").append('<div id="hershey_data" class="table-responsive"><table id="heshey_data_table" class="table_sortable"><thead><tr><th class="width_150">Transaction Date</th><th class="width_150">Lic Plate #</th><th class="width_150">Item #</th><th class="width_150">Batch #</th><th class="width_150"> Production #</th><th class="width_150">Quantity</th><th class="width_150">UOM</th> </tr></thead><tbody></tbody><tfoot></tfoot></table> </div>')
            jQuery.each(hershey.data, function(x, data) {
                $("#hershey_data tbody").append('<tr><td>' + data.TransactionDate +'</td><td><a href="' + data.file + '" id="' + data.file + '" class="edit_hershey" data-transactiondate="' + data.TransactionDate + '" data-licplate="' + data.LicPlate + '" data-timestamp="' + data.TimeStamp + '" data-item="' + data.Item + '" data-batch="' + data.Batch + '" data-production="' + data.Production + '" data-quantity="' + data.Qty + '" data-uomdenominator="' + data.UOMDenominator + '" data-uom="' + data.UOM + '" data-file="' + data.file +'">' + data.LicPlate + '</a></td><td>' + data.Item + '</td><td>' + data.Batch + '</td><td>' + data.Production + '</td><td>' + data.Qty +'</td><td>' + data.UOM +'</td></tr>');
            });
        } else {
...
function refreshHersheyDates (user_token) {
    ...
    var startDateTxt = document.getElementById("hersheyStartDatePicker");
    var endDateTxt = document.getElementById("hersheyEndDatePicker");
    $.get("/src/hershey_documents.php?user_token=" + user_token + "&start_date" + startDateTxt.value + "&end_date=" + endDateTxt.value + "&company_id=" + hold_company_id, function (result) {
        var hershey = jQuery.parseJSON(result);
        if (hershey.count > 0) {
            $("#hershey_list").html('');
            $("#hershey_list").append('<div id="hershey_data" class="table-responsive"><table id="heshey_data_table" class="table_sortable"><thead><tr><th class="width_150">Transaction Date</th><th class="width_150">Lic Plate #</th><th class="width_150">Item #</th><th class="width_150">Batch #</th><th class="width_150"> Production #</th><th class="width_150">Quantity</th><th class="width_150">UOM</th> </tr></thead><tbody></tbody><tfoot></tfoot></table> </div>')
