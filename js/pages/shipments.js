            ...
            jQuery.each(shipments.data, function(i, ship) {
                jQuery.each(ship, function(x, psId) {
                    ...
                    jQuery.each(psId, function(z, det) {
                        $("#shipments_list tbody").append('<tr><td>' + det.PackingSlipId + '</td><td>' + det.ShipDate + '</td><td class="text_right">' + det.Item + '</td><td class="text_right">' + det.Description + '</td><td>' + det.SalesOrder + '</td><td class="text_right">' + det.CustomerRef + '</td></tr>');
                    });
                });
            });
